...
def compute_lrm_block(block: cp.ndarray, *, kernel_size: int = 25,
    ...
    # Estimate local trend with NaN-aware Gaussian smoothing.
    sigma = kernel_size / 3.0
    trend, _ = handle_nan_with_gaussian(block, sigma=sigma, mode='nearest')

    # Local relief residual.
    lrm = block - trend
    ...
    if normalize:
        ...
        # Smoothly map to signed display range.
...
class LRMAlgorithm(DaskAlgorithm):
    ...
    def process(self, gpu_arr: da.Array, **params) -> da.Array:
        ...
        if not stats_ok:
            # Compute one global scale from the full array.
            stats = compute_global_stats(
                gpu_arr,
