"""
FujiShaderGPU/core/system_config.py
"""
...
def get_gpu_config(gpu_type: str = "auto", sigma: float = 10.0, multiscale_mode: bool = True, pixel_size: float = 0.5, target_distances: Optional[List[float]] = None) -> dict:
    ...
    configs = {
        ...
        "t4": {
            "tile_size": 3072,      # T4: 16GB VRAM、中程度性能
            "max_workers": min(8, sys_config["cpu_count"]),
            "padding": calculated_padding,
            "vram_monitor": False,
            "batch_size": 2,        # VRAM余裕あるが計算力控えめ
            "prefetch_tiles": 6,    # 大容量VRAMを活用
            "description": "Tesla T4 最適化（16GB VRAM、3K tiles）"
        },
        "l4": {
            "tile_size": 6144,      # L4: 24GB VRAM、高性能
            "max_workers": min(12, sys_config["cpu_count"]),
            "padding": calculated_padding,
            "vram_monitor": True,   # 大容量処理で監視
            "batch_size": 3,        # 高性能+大容量VRAM
            "prefetch_tiles": 8,    # 積極的プリフェッチ
            "description": "L4 超高速最適化（24GB VRAM、6K tiles）"
        },
        "a100": {
            "tile_size": 8192,      # A100の強力さを活用（16384→8192でより安定）
...
def detect_optimal_system_config() -> dict:
    ...
    if config["vram_gb"] >= 40:  # A100クラス
        ...
        config["optimization_level"] = "very_high"
